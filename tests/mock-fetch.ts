export function mockFetch() {
  ...
  const fetchMock = jest.fn((_input: RequestInfo | URL, _init?: RequestInit) => Promise.resolve(new Response()))
  ...
  function extractIntakeUrlAndFormDataFromFetch(callIndex = 0) {
    ...
    const [intakeUrl, request] = fetchMock.mock.calls[callIndex] as [string, { body: FormData; method: string }]
