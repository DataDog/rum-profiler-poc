...
export function resolveProfile(profile: Profile) {
  ...
  const periodType = profile.periodType
    ? {
        type: profile.stringTable.strings[profile.periodType.type as number],
        unit: profile.stringTable.strings[profile.periodType.unit as number],
      }
    : undefined
    ...
    label: sample.label.map((label) => {
      if (label.str) {
        return {
          ...
          str: label.str ? profile.stringTable.strings[label.str as number] : undefined,
        }
      } else {
        const resolvedLabel: {
          key: string
        ...
        if (label.numUnit) {
          resolvedLabel.numUnit = profile.stringTable.strings[label.numUnit as number]
        }
        return resolvedLabel
      }
    }),
    locations: sample.locationId.map((locationId) => locationsMap[locationId as number]),
    value: sample.value,
  }))

  const comments = profile.comment.map((comment) => profile.stringTable.strings[comment as number])
