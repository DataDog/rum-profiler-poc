...
import { Function, Label, Line, Location, Profile, Sample, ValueType } from './pprof-profile'
...
function buildPprof(trace: RumProfilerTrace): Blob {
  ...
  for (let frameIndex = 0; frameIndex < trace.frames.length; frameIndex++) {
    ...
    const filename = frame.resourceId !== undefined ? stringsTable.dedup(trace.resources[frame.resourceId]) : 0
    ...
    let functionIndex = functions.findIndex(
      (fn) => fn.filename === filename && fn.name === name && fn.systemName === systemName
    )
      ...
      for (const event of events) {
        labels.push(
          Label.fromPartial({
            ...
            str: stringsTable.dedup(`${event.name} (${Math.round(event.startTime)})`),
          })
        )
      }
...
function sendPprof(start: Date, end: Date, pprof: Blob, config: RumProfilerConfig): Promise<void> {
  ...
  formData.append('event', new Blob([JSON.stringify(event)], { type: 'application/json' }), 'event.json')
  formData.append('wall-time.pprof', pprof, 'wall-time.pprof')

  const buildEndpointForTransport = (transport: 'beacon' | 'fetch') =>
    buildEndpoint(config.site, config.clientToken, 'profile', [], transport)
  ...
  const didSendWithBeacon = navigator.sendBeacon(buildEndpointForTransport('beacon'), formData)
