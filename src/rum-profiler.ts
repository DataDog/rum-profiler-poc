...
const MUTATION_SAMPLE_INTERVAL_COUNT = 10; // Sample every 10 mutation
...
export class RumProfiler {
  ...
  constructor(private readonly config: RumProfilerConfig) {
    this.performanceObserver = new PerformanceObserver(this.handlePerformance);
  ...
  start(): void {
    ...
    this.performanceObserver.observe({
      entryTypes: ["longtask", "measure", "event"],
    });
  ...
  private startNextProfilerSession = (): void => {
    ...
    this.session = {
      ...
      profiler: new window_Profiler({
        ...
        maxBufferSize: Math.round(
          (COLLECT_INTERVAL_MS * 1.5) / SAMPLE_INTERVAL_MS
        ),
      }),
      timeoutId: setTimeout(
        this.startNextProfilerSession,
        COLLECT_INTERVAL_MS
      ) as unknown as number, // NodeJS types collision
      longTasks: [],
      measures: [],
      events: [],
      navigation: [],
    ...
    this.session.profiler.addEventListener(
      "samplebufferfull",
  ...
  private collectProfilerSession = (): Promise<void> => {
    ...
    this.handleEntries(this.performanceObserver.takeRecords());
    ...
    const profileReportedPromise = this.session.profiler
      .stop()
      .then((trace) => {
    ...
    this.session.profiler.removeEventListener(
      "samplebufferfull",
      this.handleSampleBufferFull
    );
  ...
  private collectNavigationEntry = (): void => {
    ...
    session.navigation.push({
      startTime: session.navigation.length
        ? session.navigation[session.navigation.length - 1].endTime
        : 0, // 0 startTime will ensure we cover activity from the session beginning
      endTime: performance.now(),
      name:
        getRum()?.getInternalContext()?.view?.name ||
  ...
  private handleMutation = (mutations: MutationRecord[]): void => {
    ...
    for (const mutation of mutations) {
      if (this.session.mutationsCount % MUTATION_SAMPLE_INTERVAL_COUNT === 0) {
        this.session.mutations.push({
          type: mutation.type,
          targetPath: getNodePath(mutation.target).join(" > "),
        });
      }
      this.session.mutationsCount++;
    }
  };
